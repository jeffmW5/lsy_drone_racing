"""Smooth gate-aware RL controller for Level 2 drone racing (exp_021).

Uses yaw-aware approach/departure waypoints (3 per gate) instead of simple
midpoints. Extracts gate yaw from obs["gates_quat"] to create smooth
fly-through trajectories. NO RETRAINING — uses exp_020 model.
"""
...
class AttitudeRL(Controller):
    """RL controller with smooth yaw-aware gate trajectory generation."""
    ...
    def __init__(self, obs: dict[str, NDArray[np.floating]], info: dict, config: dict):
        ...
        model_path = Path(__file__).parent / "ppo_drone_racing_exp020.ckpt"
    ...
    def _build_trajectory(self, gates_pos, gates_quat, drone_pos):
        """Build smooth trajectory with yaw-aware approach/departure vectors.

        For each gate, generates 3 waypoints:
          pre_gate  = gate_pos - d * [cos(yaw), sin(yaw), 0]
          gate_center = gate_pos
          post_gate = gate_pos + d * [cos(yaw), sin(yaw), 0]
        """
        ...
        d = 0.5  # approach/departure distance

        waypoints = [takeoff_pos, climbout]

        for i in range(n_gates):
            # Extract yaw from gate quaternion
            rot = Rotation.from_quat(gates_quat[i])  # scipy uses [x, y, z, w]
            yaw = rot.as_euler("ZYX")[0]  # Z-axis rotation = yaw
    ...
    def compute_control(
        ...
        if not self._trajectory_built:
            self._build_trajectory(obs["gates_pos"], obs["gates_quat"], obs["pos"])
